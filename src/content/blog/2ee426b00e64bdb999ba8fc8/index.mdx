...
description: 'npm workspacesとTurborepoの違いをよくわかっていなかったので違いを勉強してそのメモを残しておこうという記事。'
date: 2025-04-17
tags: ['npm', 'Turborepo']
...
### `node_modules`はルートにできている

つまりこのままだとマイクロサービス(例えばFaaSなど)に`a`と`b`のそれぞれをデプロイする場合にもプロジェクト全体を置く必要があることを意味する。なぜなら`node_modules`はルートにできるからだ。
...
しかし全く問題ない。結局どちらの場合もバンドラがないために起こっているからだ。というわけで今回は`esbuild`を突っ込んで改めてビルドする。
...
一方でTurborepo は `npm workspaces` や `pnpm workspaces` を**ベースにして**、追加機能を提供するツールです。
